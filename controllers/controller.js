...
const { getUser } = require("../Utilites/getUser");
...
const getCartItems = async (req, res) => {
  ...
  if (req.isLoggedIn) {
    try {
      const a = await Cart.find({ userEmail: user.email });
      const qr = a.filter((item) => {
...
const addtOCart = async (req, res) => {
  ...
  // console.log(user);
  if (!req.isLoggedIn) {
    return res.send({ msg: "Login to add product to cart" });
  }
  const { ids } = req.body;

  const product = await Product.findOne({ ids: ids });
  ...
  if (existingProduct) {
    ...
    const w = await Cart.find({
      userEmail: user.email,
      quantity: { $gt: 0 },
    });
    console.log("-------------------------");
    console.log(w.length, "wwwwwwwwwwwwww");
    return res.send({ msg: "quantity updated", item: w.length });
  }
  ...
  const t = await Cart.create(tempObj);
...
const deleteZeroQuantityItems = async (req, res) => {
  await Cart.deleteMany({ quantity: { $lt: 1 } });
};
...
const deleteItem = async (req, res) => {
  try {
    ...
    await Cart.deleteOne({ userEmail: user.email, productId: ids });
    ...
    return res.send({ msg: "Product removed from the cart", w });
  } catch (error) {
    console.log(error);
  }
};

const decreaseItemCart = async (req, res) => {
  const user = await getUser(req.headers);
  ...
  // const zero = 0;
  try {
    const zeroQuantity = await Cart.deleteOne({
      userEmail: user.email,
      productId: ids,
      quantity: 0,
    });
  ...
  // console.log(zeroQuantity);
  // await Cart.deleteOne(zeroQuantity);

  const product = await Product.findOne({ ids: ids });
  console.log(product);

  const item = await Cart.findOne({ userEmail: user.email, productId: ids });
  // console.log(item);

  const t = await Cart.updateOne(
    {
      userEmail: user.email,
      productId: ids,
    },
  ...
  res.send({
    msg: `You changed quantity of ${product.product}`,
    tempObj,
    item: w.length,
  });
};
...
const orderDetails = async (req, res) => {
  ...
  try {
    const save = await Orders.insertMany(data);

    const w = await Cart.deleteMany({ userEmail: user.email });
    console.log(w, "wwwwwwwwwwwwwwwwwwwwwwww");

    return res.send({ msg: "Order Placed", savea: save });
  } catch (err) {
    console.log(err);
...
const logOut = (req, res) => {
  try {
    ...
    return res.send({ msg: "Logged Out Succesfully", token: null });
  } catch (err) {
    console.log(err);
  }
};
